...
class AttributeNet(nn.Module):
    ...
    def __init__(self, opt, output_dim):
        ...
        self.fc_shape = nn.Sequential(
            nn.Linear(512, 128),
            nn.Linear(128, output_dim['shape'])
        )
        self.fc_color = nn.Sequential(
            nn.Linear(512, 128),
            nn.Linear(128, output_dim['color'])
        )
        self.fc_size = nn.Sequential(
            nn.Linear(512, 128),
            nn.Linear(128, output_dim['size'])
        )
        self.fc_material = nn.Sequential(
            nn.Linear(512, 128),
            nn.Linear(128, output_dim['material'])
        )
...
def get_model(opt):
    ...
    model = AttributeNet(opt, output_dim)
