...
def get_occlusion_image(objects):
    scene_depth = []
    for obj in objects:
...
def get_occlusion(objects):
    ...
    for i, obj in enumerate(objects):
        print(obj['shape_name'], obj['azimuth'])
        '''
        obj
            - occlusion
    ...
    return objects
            ...
            # rle_mask_before = coco_mask.encode(np.asfortranarray(mask_before))
            # rle_mask_after = coco_mask.encode(np.asfortranarray(mask_after))
            # rle_mask_occ = coco_mask.encode(np.asfortranarray(mask_occ))

            # rle_mask_before['counts'] = rle_mask_before['counts'].decode()
            # rle_mask_after['counts'] = rle_mask_after['counts'].decode()
            # rle_mask_occ['counts'] = rle_mask_occ['counts'].decode()
...
def render_scene(pred_scene, image_dir):
    ...
    for pred_obj in pred_scene:
        ...
        pred_obj['mesh'] = get_single_object_mesh(pred_obj['shape_name'], pred_obj['azimuth'], pred_obj['elevation'], \
                                    pred_obj['theta'], pred_obj['distance'], pred_obj['principal'], distance_rescale = distance_rescale)

    image_name = pred_scene[0]['image_filename']
    # pred_scene = get_occlusion(pred_scene)
    color_index = get_occlusion_image(pred_scene)
    os.makedirs('output_scene', exist_ok=True)
    cv2.imwrite(f"output_scene/{image_name}.png", color_index)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--anno_file', type=str, default = '/home/xingrui/vqa/superclevr-NSVQA/data/parts-nemo/reason/scene_pred-nemo.json', help='text')
    ...
    # parser.add_argument('--output_file', type=str, default = '/home/xingrui/vqa/nemo_superclevr_copy/output/json/0424_15000/pred_prob_part_test.json', help='text')
    parser.add_argument('--output_file', type=str, default = '/home/xingrui/vqa/nemo_superclevr_copy/output/json/0516_vis/pred_prob_part_test.json', help='text')
    ...
    for scene_id in tqdm(anno):
        # print(scene_id)
        if int(scene_id) < 31818:
            continue
        if int(scene_id) > 32105:
            continue
        pred_scene = anno[scene_id]
        render_scene(pred_scene, args.image_dir)
