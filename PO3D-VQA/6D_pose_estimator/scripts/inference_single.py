...
def main():
    ...
    object_score_dict = {}
    i = args.test_index
    sample = dataset.__getitem_by_id__(i)
    ...
    print("args.pre_filter", args.pre_filter)
    if args.pre_filter:
        with torch.no_grad():
            ...
            print("Size: kpt_score_map ", kpt_score_map.size())
        ...
        print("Size: feature_map ", feature_map.size())
        print('kp_features', kp_features.size())
        # print('clutter_bank', clutter_bank.size())
        pred = solve_pose_multi_obj(
            feature_map, inter_module, kp_features, clutter_bank, poses, kp_coords, kp_vis,
            epochs=args.epochs,
            lr=args.lr,
    ...
    save_segmentation_maps(pred['seg_maps'], os.path.join(args.save_results, sample['img_name'], f'segmentation.png'), down_sample_rate=args.down_sample_rate)
    seg_end = time.time()
