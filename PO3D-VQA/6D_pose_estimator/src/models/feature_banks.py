...
def mask_remove_near(keypoints, thr, dtype_template=None, num_neg=0, neg_weight=1, eps=1e5):
    ...
    with torch.no_grad():
        ...
        if num_neg == 0:
            return ((distance <= thr.unsqueeze(1).unsqueeze(2)).type_as(dtype_template) - torch.eye(keypoints.shape[1]).type_as(dtype_template).unsqueeze(dim=0)) * eps
        else:
            tem = (distance <= thr.unsqueeze(1).unsqueeze(2)).type_as(dtype_template) - torch.eye(keypoints.shape[1]).type_as(dtype_template).unsqueeze(dim=0)
